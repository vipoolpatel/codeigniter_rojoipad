...
<div class="gksel_navigation"><a href="<?php echo str_replace('rojoipad/', '', base_url()) ?>rojoipad/index.php/admins/training_center/content_category_list/<?=$sub_category_id?>">Sub Category Content</a> &gt; Edit Sub Category Content</div>
        ...
        <table class="gksel_normal_tabpost">
            ...
            <tr>
               <td align="right" width="150">Add Link</td>
                <td align="left">
                    <input type="text" name="add_link"  id="add_link_pre" value="<?=$getRecord->add_link?>" style="width:90%;" required=""/>
                    <div class="tipsgroupbox"><div class="request">*</div></div>
                </td>
            </tr>

            <tr>
                <td align="right" width="150" valign="top">Preview</td>
                <td align="left">
                <div style="width:100px;height:100px;background: #CCC;position: relative;margin-bottom: 6px;" id="get_add_link_image">
               
                </div>
                
                <button type="button" class="nav_on" id="img_preview_link" style="border: none;" ><img class="plus" src="<?php echo base_url().'themes/default/images/week_gou.png'?>"/></button>
                </td>
   ...
   $('body').delegate('#img_preview_link', 'click',function(){
    var add_link = $("#add_link_pre").val();
        //alert(add_link);
        $.ajax({
            url: "<?php echo str_replace('rojoipad/', '', base_url()) ?>rojoipad/index.php/admins/training_center/preview_update_link",
            type: "POST",
            dataType: "json",
            data: {add_link: add_link},
            success: function(data){
                $('#get_add_link_image').html(data.success);
            }
        });
   });
