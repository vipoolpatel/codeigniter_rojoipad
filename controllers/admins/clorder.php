...
class Clorder extends CI_Controller {
    ...
    function index() {
        ...
        $data['orderlist'] = $this->ClorderModel->getorderlist($con);
        $data['count'] = $this->ClorderModel->getorderlist($con, 1);
        $url = base_url() . 'index.php/admins/clorder/index?status=' . $status . '&keyword=' . $keyword . '&page=' . $page;
    ...
    function toview_order($order_id) {
        ...
        if ($backurl != "") {
            ...
            if (base64_decode($backurl) != "") {
                ...
                $decodebackurl = base_url() . 'index.php/admins/clorder/index';
            }
        } else {
            $decodebackurl = base_url() . 'index.php/admins/clorder/index';
        }
        ...
        if ($this->langtype == '_ch') {
            ...
            $order_view_text = 'View Clorder';
        }
        //导航栏
        $data['url'] = '<a href="' . $decodebackurl . '">' . lang('dz_orders') . '</a> &gt; ' . $order_view_text;

        $data['orderinformation'] = $this->ClorderModel->getorderinfo($order_id);
    ...
    function del_order($order_id) {
        $this->ClorderModel->del_order($order_id);
// 		redirect('admins/clorder/index');
    }
    ...
    function edit_order($order_id) {
        ...
        $this->ClorderModel->del_order_details($order_id);
        $count = 0;

        $sql = $this->db->query("select order_list_id,design_data_15,design_data_16,design_data_17,design_data_18,design_data_19 from " . DB_PRE() . "order_list where o_id=" . $order_id);
        foreach ($sql->result_array() as $data) {
            ...
            $arr['code_suit_select'] = trim($code_suit_select);
            $arr['code_waistcoat_select'] = trim($code_waistcoat_select);
            $arr['code_trousers_select'] = trim($code_trousers_select);
            $arr['code_shirt_select'] = trim($code_shirt_select);
            $arr['code_overcoat_select'] = trim($code_overcoat_select);
            ...
            for ($j = 0; $j < count($alldesign); $j++) {
                ...
                //if ($this->input->post('design_id_' . $design_id . '_' . $order_list_id) != '') {
                    ...
                    $designarr['input_value'] = trim($input_value);
                    $designarr['checkbox_value'] = trim($checkbox_value);
                    $designarr['radio_value'] = trim($radio_value);
                    $designarr['input2_value'] = trim($input2_value);
                    
                    $jsonarr['design_id'] = trim($design_id);
                    $jsonarr['input_value'] = (trim($input_value) != '') ? $input_value:"";
                    $jsonarr['checkbox_value'] = (trim($checkbox_value) != '') ? true:false;
                    $jsonarr['radio_value'] = (trim($radio_value) != '') ? $radio_value:false;
                    $jsonarr['input2_value'] = (trim($input2_value) != '') ? $input2_value:"";
                    
                    $jsonarr1[] = $jsonarr; 
					
					if($category_id > 0){
					$this->ClorderModel->add_order_detail($designarr);	
					}
            ...
            $arr['design_data_'.$category_id1] = json_encode($jsonarr1);
            
            $this->ClorderModel->edit_order($order_list_id, $arr);
        }

        $arr1 = array('edited' => time());
        $this->ClorderModel->edit_main_order($order_id, $arr1);

        $orderinfo = $this->ClorderModel->getorderinfo($order_id);
        if (!empty($orderinfo)) {
            $client_realname = $this->input->post('client_realname');
            $client_address = $this->input->post('client_address');
            $client_email = $this->input->post('client_email');
    ...
    function tocompleted_order($order_id) {
        //$arr = array();
        $arr['status'] = 5;
        $this->ClorderModel->edit_order_status($order_id, $arr);
    }
