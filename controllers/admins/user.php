...
class User extends CI_Controller {
    ...
    function wallet() {
        ...
        $con = array('orderby' => 'u.user_number', 'orderby_res' => 'DESC');
        $con['row'] = $row;
        $con['page'] = $data['page'];
        ...
        $data['walletdetails'] = $this->UserModel->getuser($con);
        /*echo '<pre>';
        print_r($data['walletdetails']);
        exit;*/
        $data['count'] = $this->UserModel->getuser($con,1);
		
		if($user_brandname != ''){
			$url = base_url() . 'index.php/admins/user/wallet?user_brandname="'.$user_brandname.'"&user_type=' . $user_type . '&keyword=' . $keyword . '&page=' . $page;
        }else{
			$url = base_url() . 'index.php/admins/user/wallet?user_type=' . $user_type . '&keyword=' . $keyword . '&page=' . $page;
		}
        ...
        print_r($data['fy']);
        exit;*/
        $this->load->view('admin/user_wallet_list', $data);
    }
    
    function toadd_user_wallet(){
        $backurl = base_url().'index.php/admins/user/wallet';
        $data['userinfo'] = $this->UserModel->getusers();
        /*echo '<pre>';
        print_r($data['user']);
        exit;*/
    ...
    function view_wallet($uid){
        $data['walletinfo'] = $this->UserModel->getwalletinfo($uid);
        /*echo '<pre>';
        print_r($data['walletinfo']);
        exit;*/
    ...
    function index() {
		...
		$user_brandname = $this->input->get('user_brandname');
        if ($user_type == 1) {
        ...
        $url = base_url() . "index.php/admins/user/index?user_brandname='".$user_brandname."'&user_type=" . $user_type . "&keyword=" . $keyword . "&page=" . $page;
        $data['fy'] = fy_backend($data['count'], $row, $url, $data['page'], 5, 2);
        $data['user_type'] = $user_type;
        if ($is_excel != 1) {
    ...
    function add_user($user_type = 0) {
        ...
        if ($uid > 0) {
            $res = $this->UserModel->add_user_details($newarr, $uid);
        }
    ...
    function toedit_user($user_type, $uid) {
        ...
        $data['usersubinfo'] = $this->UserModel->getusersubinfo($uid);
        /* echo '<pre>';
          print_r($data['usersubinfo']);
          exit; */
        $this->load->view('admin/user_edit', $data);
    }
	
	
	function toedit_client_user($user_type, $uid) {
        //跳转到列表页面
        $backurl = $this->input->get('backurl');
        if ($backurl != "") {
            $backurl = str_replace('slash_tag', '/', $backurl);
        ...
        /* echo '<pre>';
          print_r($data['usersubinfo']);
          exit; */
        $this->load->view('admin/user_edit_client', $data);
    }
    
    function edit_user_admin(){
        $backurl = $this->input->post('backurl'); //昵称
        $uid = $this->input->post('uid'); //昵称
        $points = $this->input->post('points'); //昵称
        $category = $this->input->post('category'); //昵称
        $design = $this->input->post('design'); //昵称
        
        $newarr = array();
        $newarr['cust_points'] = $points;
        $newarr['category'] = $category;
        $newarr['design'] = $design;
        
        $res = $this->UserModel->add_user_details_design($newarr, $uid);
    ...
    function edit_user($uid) {
		...
		if($user_id > 0){
			$decodebackurl = base_url() . 'index.php/admins/client/index?user_type=1';
		}
    ...
    function save_wallet() {
        //用户信息
        $userid = $this->input->post('userid'); //用户名
        $amount = $this->input->post('amount'); //手机号码
        $comment = $this->input->post('comment'); //邮箱
        
        //跳转到列表页面
        $backurl = $this->input->post('backurl');
        
        $arr = array('userid' => $userid, 'amount' => $amount, 'comment' => $comment);
        
        
        $user_id = $this->UserModel->add_wallet($arr);
    ...
    function toreaction_clientno_fromorder($order_id) {
        ...
        $clientinfo = $this->UserModel->getuserinfo($client_id);
