...
class OrderModel extends CI_Model{
	...
	function getorderlist($con = array(),$iscount=0){
		...
		if(isset($con['keyword'])){if($where!=""){$where .=" AND";}else{$where .=" WHERE";} $where .= " (u.user_realname like '%".$con['keyword']."%' or aaaa.user_realname like '%".$con['keyword']."%' or aaaa.user_number like '%".$con['keyword']."%' or c.user_realname like '%".$con['keyword']."%' or d.category_name_en like '%".$con['keyword']."%' or o.order_number like '%".$con['keyword']."%')";}
	...
	*/
	...
	function edit_order_detail($id,$arr){
		$this->db->update(DB_PRE().'order_detail',$arr,array('id'=>$id));
	}
		...
		if($query->num_rows()>0){ 
			...
			if(!empty($categorylist)){for ($c = 0; $c < count($categorylist); $c++) {
				...
				$sql = "
						...
						WHERE a.order_id = ".$order_id." AND a.category_id = ".$category_id."
				...
				$orderinfo['design_list_'.$category_id] = $subarr;
	...
	function del_order($order_id){
		...
		if(!empty($orderinfo)){
			...
			//同时删除订单的详细产品--结束
			$this->db->delete(DB_PRE().'user_list',array('uid'=>$orderinfo['client_id']));
			...
			$this->db->delete('ipadqrcode_product_list',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_step1',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_step2',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_step3',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_step4',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_step5',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_form_meacheck',array('product_id'=>$orderinfo['qrcode_product_id']));
			$this->db->delete('ipadqrcode_product_form_qfcheck',array('product_id'=>$orderinfo['qrcode_product_id']));
	...
	function del_gksel_order_detail($detail_id=0){
		$this->db->delete(DB_PRE().'order_detail',array('detail_id'=>$detail_id));
	}
	...
	function getuserorders($uid){
		...
		return $this->db->get_where(DB_PRE().'order_list',array('uid'=>$uid))->result_array();
	}
	...
	function getexpress_select($id=0){
		$str='';
		$express=$this->ExpressModel->getexpresslist(array('status'=>1,'orderby'=>'e.id','orderby_res'=>'ASC'));
		if(!empty($express)){
			for($i=0;$i<count($express);$i++){
				if($id==$express[$i]['id']){$isselect=' selected';}else{$isselect='';}
	...
	function updateorder_soldnum($order_id = 0){
		$order_detaillist = $this->OrderModel->getorder_productlist(array('order_id'=>$order_id, 'orderby'=>'id', 'orderby_res'=>'ASC'));
	...
	function order_minusinventory($order_id){
		$orderinfo = $this->OrderModel->getorderinfo($order_id);
		//减少库存 ------ START
		$order_detaillist = $this->OrderModel->getorder_productlist(array('order_id'=>$order_id,'orderby'=>'id','ASC'));
		if(!empty($order_detaillist)){
			for ($dddd = 0; $dddd < count($order_detaillist); $dddd++) {
				...
				if(!empty($inventoryinfo)){
					...
					$this->db->insert(DB_PRE().'product_inventory_list', array('product_id'=>$product_id, 'size_id'=>$size_id, 'inventory_num'=>0));
					$inventory_id = $this->db->insert_id();
					$old_inventory_num = 0;
				}
				if($old_inventory_num >= $action_num) {

                    $this->db->insert(DB_PRE() . 'product_inventory_detail', array('action_type' => 3, 'inventory_id' => $inventory_id, 'action_num' => $action_num, 'action_desc' => $action_desc, 'action_time' => time()));
					...
					if(!empty($info)){
						$this->db->update(DB_PRE().'product_inventory_total', array('inventory_totalnum'=>$totalnum), array('inventory_totalid'=>$info['inventory_totalid']));
					}else{
						$this->db->insert(DB_PRE().'product_inventory_total', array('product_id'=>$product_id, 'inventory_totalnum'=>$totalnum));
						$this->db->insert_id();
					}
				}
			}
