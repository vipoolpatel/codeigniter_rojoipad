...
class UserModel extends CI_Model {
    ...
    function getusersubinfo($uid) {
        $sql = "select * from " . DB_PRE() . "user_details where userid=" . $uid;
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
    ...
    function add_user_details($arr, $uid) {
        //$this->db->delete(DB_PRE() . 'user_details', array('userid' => $uid));
		
		
        if(count($arr['points']) > 1){
				for ($i = 0; $i < count($arr['points']); $i++) {
					$data = array();
					
					 $data['points'] = $arr['points'][$i];
					$data['userid'] = $uid;
					$data['cat_id'] = $arr['category'][$i];
					$data['design_id'] = $arr['design'][$i];
						
					$sql = $this->db->query("select id from " . DB_PRE() . "user_details where userid=" . $uid . " and cat_id=" . $arr['category'][$i] . " and design_id=" . $arr['design'][$i]);
					$num_rows = $sql->num_rows();

					if ($num_rows > 0) {
						$this->db->update(DB_PRE() . "user_details", array('points' => $arr['points'][$i]), array('userid' => $uid,'cat_id'=> $arr['category'][$i],'design_id'=>$arr['design'][$i]));
					} else {
						$this->db->insert(DB_PRE() . 'user_details', $data);
					}
				}
		}
        return '1';
    ...
    function add_user_details_design($arr, $uid) {
        //$this->db->delete(DB_PRE() . 'user_details', array('userid' => $uid));

        for ($i = 0; $i < count($arr['cust_points']); $i++) {
            $data = array();
            
             $data['cust_points'] = $arr['cust_points'][$i];
                $data['userid'] = $uid;
                $data['cat_id'] = $arr['category'][$i];
                $data['design_id'] = $arr['design'][$i];
            ...
            if ($num_rows > 0) {
                $this->db->update(DB_PRE() . "user_details", array('cust_points' => $arr['cust_points'][$i]), array('userid' => $uid,'cat_id'=> $arr['category'][$i],'design_id'=>$arr['design'][$i]));
            } else {
                $this->db->insert(DB_PRE() . 'user_details', $data);
            }
        }
    ...
    function getuserlist($con = array(), $iscount = 0) {
        ...
        if (isset($con['username'])) {
            if ($where != "") {
                $where .= " AND";
            } else {
                $where .= " WHERE";
            } $where .= " u.wechat_nickname LIKE '%" . addslashes($con['username']) . "%'";
        }
        if (isset($con['keyword'])) {
            if ($where != "") {
                ...
                $where .= " WHERE";
            } $where .= " ((u.user_phone LIKE '%" . addslashes($con['keyword']) . "%') OR (u.user_realname LIKE '%" . addslashes($con['keyword']) . "%') OR (u.user_number LIKE '%" . addslashes($con['keyword']) . "%')) ";
        }
        if (isset($con['parent'])) {
            if ($where != "") {
                $where .= " AND";
                ...
                $where .= " WHERE";
            } $where .= " u.parent = " . $con['parent'];
        }
        if (isset($con['user_type'])) {
            if ($where != "") {
                $where .= " AND";
            } else {
                $where .= " WHERE";
            } $where .= " u.user_type = " . $con['user_type'];
        }
    ...
    function getuser_pointsetting_historylist($con = array(), $iscount = 0) {
        ...
        if (isset($con['pointsetting_id'])) {
            if ($where != "") {
                $where .= " AND";
            } else {
                $where .= " WHERE";
            } $where .= " pointsetting_id =" . $con['pointsetting_id'];
        }
        if (isset($con['orderby']) && isset($con['orderby_res'])) {
            $order_by .= " ORDER BY " . $con['orderby'] . " " . $con['orderby_res'] . "";
        }
    ...
    //添加用户地址
    function add_wallet($arr) {
        $this->db->insert(DB_PRE() . 'user_wallet', $arr);
        return $this->db->insert_id();
    }
    ...
    function get_user_details($uid) {
        $sql = $this->db->query("select id from " . DB_PRE() . "user_details where userid= " . $uid);
        $num_rows = $sql->num_rows();
        return $num_rows;
    }

    function getuserwalletinfo() {
        $sql = $this->db->query("select w.*,ul.user_realname from " . DB_PRE() . "user_wallet w left join " . DB_PRE() . "user_list ul on w.userid=ul.uid order by ul.uid ");
        $num_rows = $sql->num_rows();
        if ($num_rows > 0) {
            $result = $this->db->query($sql)->result_array();
            return $result;
    ...
    function getuser($con = array(), $iscount = 0) {
        ...
        if ($iscount == 0) {
            $sql = "SELECT u.*,sum(uw.amount) as amount FROM " . DB_PRE() . "user_list u left join " . DB_PRE() . "user_wallet uw on u.uid=uw.userid $where $order_by $limit";
            $result = $this->db->query($sql)->result_array();
            if (!empty($result)) {
                return $result;
            } else {
            ...
            $sql = "SELECT count(uw.userid) as count FROM " . DB_PRE() . "user_list u left join " . DB_PRE() . "user_wallet uw on u.uid=uw.userid $where and uw.amount> 0 $order_by ";
            $result = $this->db->query($sql)->row_array();
            if (!empty($result)) {
                return $result['count'];
            } else {
    ...
    function getusers() {
        ...
        $sql = $this->db->query("select uid,user_realname from " . DB_PRE() . "user_list order by uid asc");
        $num_rows = $sql->num_rows();
        if ($num_rows > 0) {
    ...
    function getwalletinfo($uid) {

        $sql = $this->db->query("select * from " . DB_PRE() . "user_wallet where userid=" . $uid . " order by id desc");
        $num_rows = $sql->num_rows();
        if ($num_rows > 0) {
            $result = $sql->result_array();
            return $result;
